class Program
{
    ...
    static void FiveOctave(int[] Sound)
    {
        Console.WriteLine("Пятая октава, белые клавиши: a-h; чёрные клавиши: q-y. Для возвращения к выбору октав, клавиша: Tab.");
        for (ConsoleKeyInfo sound = Console.ReadKey(); sound.Key != ConsoleKey.Tab;)
        {
            if (sound.Key == ConsoleKey.A)
            {
